...
OUTPUT_DIRECTORY_PATH = '/images'  # Directory for output images
NUM_OF_SEMAPHORE = 5  # The number of semaphores
URL_CHUNK_SIZE = 10  # The chunk containing the number of URLs


if __name__ == '__main__':
    fileManager = FileManager(INPUT_FILE_PATH, URL_CHUNK_SIZE)
    spider = Spider(OUTPUT_DIRECTORY_PATH, NUM_OF_SEMAPHORE)
    loop = asyncio.get_event_loop()

    url_generator = fileManager.get_image_url()
    if url_generator is not None:
        for urls in url_generator:
            tasks = [spider.download(url) for url in urls]
